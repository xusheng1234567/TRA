...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch, total_epochs):
        ...
        overall_outs = []
        overall_trgs = []
        for batch_idx, (data, target,data_retrieval,target_retrieval) in enumerate(self.data_loader):
            data, target,data_retrieval,target_retrieval = data.to(self.device), target.to(self.device),data_retrieval.to(self.device),target_retrieval.to(self.device)
            self.optimizer.zero_grad()
            output = self.model(data,data_retrieval,target,target_retrieval,"train")
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for (batch_idx, (data, target, data_retrieval, target_retrieval)) in enumerate(self.valid_data_loader, ):
                data, target,data_retrieval,target_retrieval = data.to(self.device), target.to(self.device),data_retrieval.to(self.device),target_retrieval.to(self.device)

                target1=target
                for i in range(int(len(target)*0.06)):
                    if target1[i]==4:
                        target1[i]=0
                    else:
                        target1[i]+=1
                output = self.model(data,data_retrieval,target1,target_retrieval,"train")
                output = self.model(data,data_retrieval,target,target_retrieval,"valid")
                loss = self.criterion(output, target, self.class_weights, self.device)
